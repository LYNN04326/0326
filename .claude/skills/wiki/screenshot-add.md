# screenshot-add — 截圖加標註（預設做法）

> 原始碼：[`.claude/skills/screenshot-add/SKILL.md`](../screenshot-add/SKILL.md)　·　舊名：`photo`（曾短暫叫過 `shot-overlay`）
...
做不到的只有一件事：**換掉圖片裡原本的文字**（HTML 只能疊在上面）。那種情況才轉 [screenshot-replace](screenshot-replace.md)。
...
本頁對應的 `SKILL.md` 是安裝時**依 screenshot-replace 的交叉引用重建**的（原始壓縮檔沒附這支）。
