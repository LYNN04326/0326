# shot-burn — 截圖像素標註（舊版工作流程）

> 原始碼：[`.claude/skills/shot-burn/SKILL.md`](../shot-burn/SKILL.md)　·　舊名：`png`

## 一句話

用 Pillow 把 badge／覆蓋文字**燒進**截圖像素。**非預設**，只在 [shot-overlay](shot-overlay.md) 辦不到時使用。
...
版面標準直接沿用 shot-overlay 規則二，**不因為「這次輸出成 PNG」就降級**。
