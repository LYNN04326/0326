...
description: >
  ...
  「規格書 UI 截圖標號慣例」時才改用 `screenshot-replace` skill（Pillow 燒進像素，舊版做法）。
---

# 截圖加標註（screenshot-add，預設做法）
...
`.claude/skills/screenshot-replace/SKILL.md`。兩者分工表見該 skill 結尾。
...
（`screenshot-replace` skill 的「整段內容輸出成單一 PNG」直接沿用本節）。
...
問題，見 `.claude/skills/screenshot-replace/SKILL.md`〈整段內容輸出成單一 PNG〉的事故記錄。
...
* 截圖裁切到 UI 元件本身邊界，不含周圍空白背景；badge 不可壓在畫面元素上，必要時在元件上緣
  補一條剛好容納 badge 的留白（做法見 `screenshot-replace` skill）。
